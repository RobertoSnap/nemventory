<template>
    <div class="col-md-12 purchase-order" v-if="order && warehouses[order.vendor_warehouse_id]">
        <b-form @submit.prevent="onSubmit" @keydown="form.errors.clear($event.target.name)">
            ...
            <!-- Actions-->
            <div class="col-md-12">
                <b-button type="submit" variant="primary" class="btn btn-primary pull-right">Update purchase order
                </b-button>
            </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.getData();
            axios.get('/api/user/purchase-order/' + this.id )
                .then(response => {
        ...
        methods: {
            ...
            onSubmit() {
                ...
                this.form.put('/api/user/purchase-order/' + this.id)
                        .then(response => {
